...
export const createPost = async (req, res) => {
  try {
    ...
    if (!text && !img) {
      return res.status(400).json({ error: "a text or image required" });
    }
    ...
    res.status(500).json({ error: "Internal Error At createPost" });
...
export const deletePost = async (req, res) => {
  try {
    ...
    if (post.user.toString() !== req.user._id.toString()) {
      return res
        .status(400)
        .json({ error: "unauthorised user cannot delete the post" });
    }
